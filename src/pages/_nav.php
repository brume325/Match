...
<div class="topbar">
  ...
  <a href="/index.php" class="brand">
    <img src="assets/img/logo-match-moov.svg" alt="MATCH MOOV" class="site-logo">
  </a>
  <div class="topbar-actions">
    <?php if ($_nav_logged): ?>
      <a href="/recherche.php" class="topbar-link<?= $_nav_current==='recherche.php'?' active':'' ?>">Rechercher</a>
      <a href="/cree_activite.php" class="topbar-link<?= $_nav_current==='cree_activite.php'?' active':'' ?>">Creer une activite</a>
      <a href="/parametres.php" class="topbar-link<?= $_nav_current==='parametres.php'?' active':'' ?>">Parametres</a>
      <a href="/recherche.php" class="topbar-link notif-btn" title="Notifications">
        Notifications<?php if ($_nav_notif_count > 0): ?><span class="notif-badge"><?= $_nav_notif_count ?></span><?php endif; ?>
      </a>
      <a href="/recherche.php" class="topbar-link notif-btn" title="Relations">
        Relations<?php if ($_nav_ami_count > 0): ?><span class="notif-badge" style="background:var(--mm-yellow);color:var(--mm-dark);"><?= $_nav_ami_count ?></span><?php endif; ?>
      </a>
      <a href="/logout.php" class="topbar-link">Deconnexion</a>
    <?php else: ?>
...
<aside id="side" class="side" aria-hidden="true">
  ...
  <nav class="side-nav">
    ...
    <?php if ($_nav_logged): ?>
      <a href="/cree_activite.php" <?= $_nav_current==='cree_activite.php'?' class="active"':'' ?>>Creer une activite</a>
      <a href="/parametres.php" <?= $_nav_current==='parametres.php'?' class="active"':'' ?>>Parametres</a>
      <a href="/recherche.php">Notifications <?php if($_nav_notif_count>0): ?><span style="background:var(--mm-blue);color:#fff;border-radius:999px;padding:1px 7px;font-size:.72rem;font-weight:700;"><?= $_nav_notif_count ?></span><?php endif; ?></a>
      <a href="/recherche.php">Relations <?php if($_nav_ami_count>0): ?><span style="background:var(--mm-yellow);color:var(--mm-dark);border-radius:999px;padding:1px 7px;font-size:.72rem;font-weight:700;"><?= $_nav_ami_count ?></span><?php endif; ?></a>
      <hr>
