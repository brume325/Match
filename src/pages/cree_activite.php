...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['image_fichier']['name'])) {
        ...
        if (!in_array($mime, $allowed)) {
            ...
            $uploadDir  = dirname(__DIR__, 2) . '/public/uploads/activites/';
            if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);
            if (move_uploaded_file($_FILES['image_fichier']['tmp_name'], dirname(__DIR__, 2) . '/public/' . $filename)) {
                $image = $filename;
            } else {
...
<html lang="fr">
  <head>
    ...
    <link rel="stylesheet" href="/assets/css/style_cree_activite.css">
  </head>
  <body>
